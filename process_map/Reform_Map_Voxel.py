...
def interpolate_fast(data,data_new,size,iterator,prev_voxel_size):
...
def interpolate_slow(data,data_new,size,iterator,prev_voxel_size):
    for i in range(1, iterator, 1):
        if(i%10==0):
            print("Finished %d",i)
        for j in range(1, iterator, 1):
...
def Reform_Map_Voxel_Final(map_path,new_map_path):
    ...
    if not os.path.exists(new_map_path):
        with mrcfile.open(map_path) as mrc:
            ...
            it_val_x = int(np.floor(size[0] * prev_voxel_size_z))
            it_val_y = int(np.floor(size[1] * prev_voxel_size_y))
            it_val_z = int(np.floor(size[2] * prev_voxel_size_x))
            ...
            for i in range(it_val_x):
                ...
                for j in range(it_val_y):
                    for k in range(it_val_z):
                        current_query=np.array([i/prev_voxel_size_z,j/prev_voxel_size_y,k/prev_voxel_size_x])
    ...
    if not os.path.exists(new_map_path):
        with mrcfile.open(map_path) as mrc:
            ...
            if (prev_voxel_size < 1):
                ...
                exit(1)
            it_val = int(np.floor(size[0] *prev_voxel_size))
            print("Previouse size:",size[0]," Current map size:",it_val)
            data_new = np.zeros([it_val,it_val,it_val])
            data_new[0, 0, 0] = data[0, 0, 0]
            data_new[it_val - 1, it_val - 1, it_val - 1] = data[
            ...
            try:
                data_new=interpolate_fast(data,data_new,size,it_val,prev_voxel_size)
            except:
                data_new = np.zeros([it_val, it_val, it_val])
                data_new[0, 0, 0] = data[0, 0, 0]
                data_new[it_val - 1, it_val - 1, it_val - 1] = data[
                    size[0] - 1, size[1] - 1, size[2] - 1]
                data_new = interpolate_slow(data, data_new, size, it_val, prev_voxel_size)
            data_new = np.swapaxes(data_new, 0, 2)
            data_new=np.float32(data_new)
            mrc_new = mrcfile.new(new_map_path, data=data_new, overwrite=True)
            vsize = mrc_new.voxel_size
